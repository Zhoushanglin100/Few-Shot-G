...
class DeepInversionFeatureHook():
    ...
    '''
    def __init__(self, name, module, mean_dict, var_dict):
    ...
    def hook_fn(self, module, input, output):
        ...
        batch_mean = self.mean_dict["module."+self.name].cuda()
        batch_var = self.var_dict["module."+self.name].cuda()
        #forcing mean and variance to match between two distributions
        #other ways might work better, i.g. KL divergence

        # print('module.running_var.data: ', module.running_var.data)
        #print('var: ', var)
        #print('real_var: ',self.var_dict[self.name[:9]+"module."+self.name[9:]])
        #print('var2d:', var.view(1,-1))
        #print('real_var2d: ',self.var_dict[self.name[:9]+"module."+self.name[9:]].view(1,-1))
        #exit(0)
        #r_feature = torch.norm(module.running_var.data - var, 2) + torch.norm(module.running_mean.data - mean, 2)
        #print(self.name)
        #print(self.var_dict["module."+self.name])
        #exit(0)
        
        # RMSE
        #r_feature = torch.norm(self.var_dict["module."+self.name] - var, 2) + torch.norm(self.mean_dict["module."+self.name] - mean, 2)
        #r_feature = torch.norm(self.var_dict[self.name[:9]+"module."+self.name[9:]] - var, 2) + torch.norm(self.mean_dict[self.name[:9]+"module."+self.name[9:]] - mean, 2)
        #print(r_feature)
        #exit(0)
        ...
        #r_feature = criterion(self.mean_dict[self.name[:9]+"module."+self.name[9:]].view(1,-1), mean.view(1,-1), torch.ones(1).cuda())+criterion(self.var_dict[self.name[:9]+"module."+self.name[9:]].view(1,-1), var.view(1,-1), torch.ones(1).cuda())
...
class DeepInversionClass(object):
    def __init__(self, bs=84,
        ...
        mean_layers_dictionary = torch.load("/home/jiachao/data/DongkuanXu/DeepInversion_Sung/get_mean_var/mean_resnet.pth")
        var_layers_dictionary = torch.load("/home/jiachao/data/DongkuanXu/DeepInversion_Sung/get_mean_var/var_resnet.pth")
        for name, module in self.net_teacher.named_modules():
             #if ("bn" in name) or ("downsample.1" in name):
             if isinstance(module, nn.BatchNorm2d):
             #if isinstance(module,nn.Conv2d):
                 self.loss_r_feature_layers.append(DeepInversionFeatureHook( name, module, mean_layers_dictionary, var_layers_dictionary))
    ...
    def get_images(self, net_student=None, targets=None):
        ...
        for lr_it, lower_res in enumerate([2, 1]):
            ...
            # print("lr_it, lower_res: ", lr_it, lower_res)

            if lr_it==0:
                iterations_per_layer = 2000 # 2000
            else:
                iterations_per_layer = 1000 if not skipfirst else 8000 # iterations_per_layer = 1000 if not skipfirst else 2000
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                if 1: # local_rank==0:
                    if iteration % save_every==0:
                        ...
                        print("loss_r_feature", loss_r_feature.item()) # print("loss_r_feature", loss_r_feature.item())
