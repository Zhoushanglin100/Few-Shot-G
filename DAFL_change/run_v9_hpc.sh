...
if [ "$flag_s2" = "1" ]; then 
    srun -p $partion --gres=gpu:1 -n 1 --cpus-per-task=4 --exclude=asimov-157 python3 DAFLDeepinvert-train_v9.py \
                                        --dataset cifar10 \
                                        -a $arch_t \
                                        --fix_G \
                                        --train_G \
...
if [ "$flag_s3" = "1" ]; then 
    srun -p $partion --gres=gpu:4 -n 1 --cpus-per-task=16 --exclude=asimov-157 python3 DAFLDeepinvert-train_v9.py \
                                        --dataset cifar10 \
                                        -a $arch_t \
                                        --arch_s $arch_s \
                                        --fix_G \
