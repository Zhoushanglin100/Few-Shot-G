...
class FSL(pl.LightningModule):
    def __init__(self, generators, teacher, student, optimizer, lr_scheduler, val_dataloaders, gen_ratio):
        super().__init__()
...
def main():
    ...
    # generator_list = []
    # for i in range():
    #     generator_list.append(Generator(img_size=args.img_size, n_channels=args.n_channels, latent_dim=args.latent_dim))
    # generators.load_state_dict('xxxxxxxxxxxxxxx')
    ...
    gen_info = utils.GeneratorInfo(args)
    gen_info.get_generators()
    generators = GeneratorList(gen_info.G_list, args.batch_size)
    ...
    pipeline = FSL(generators, teacher, student, optimizer, lr_scheduler, [val_dataloader], args.ratio) 
    trainer = pl.Trainer(max_steps=1000000, log_every_n_steps=100, 
    ...
    # if args.eval_only:
    #     trainer.test(pipeline, ckpt_path="best", dataloaders=val_dataloader)
    # else:
    print('start train')
