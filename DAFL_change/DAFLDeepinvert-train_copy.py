...
def adjust_learning_rate(optimizer, epoch, learning_rate):
    ...
    #    lr = 0
    if epoch < 300: # warn up
        #lr = learning_rate * (epoch-100)/ 200
        lr = learning_rate * (epoch)/300
    else:
        lr = learning_rate # lr decay
        lr_sq = ((epoch-300 )// 30)+1
...
# images from true dataset
net.train()
for epoch in range(opt.n_epochs):
    avg_loss = 0.0
    total_correct = 0
    adjust_learning_rate(optimizer_S, epoch * 10,opt.lr_S)
    for i, (images, labels) in enumerate(data_train_loader):
        ...
        #loss_kd = kdloss(net(images), outputs_T.detach())
        ...
        optimizer_S.step()
        if i % 10 == 0:
            print ("[Epoch-batch %d/%d] [loss_oh: %f] [loss_kd: %f]" % (epoch, i,loss.item(), loss_kd.item()))
    with torch.no_grad():
        for i, (images, labels) in enumerate(data_test_loader):
            images = images.cuda()
            labels = labels.cuda()
